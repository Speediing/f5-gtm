...
fn agentComputerMark(p: vec2f) -> f32 {
  ...
  let screen = sdRoundedBox(q - vec2f(0.0, -0.015), vec2f(0.13, 0.09), 0.014);
  let bezel = sdRoundedBox(q - vec2f(0.0, -0.015), vec2f(0.145, 0.105), 0.018);
  var mark = opSmoothUnion(plate, bezel, 0.01);
  mark = opSmoothSub(mark, screen, 0.008);
  let stand = sdRoundedBox(q - vec2f(0.0, 0.105), vec2f(0.035, 0.04), 0.008);
  let base = sdRoundedBox(q - vec2f(0.0, 0.145), vec2f(0.09, 0.012), 0.006);
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let red = vec3f(0.894118, 0.0, 0.168627);
